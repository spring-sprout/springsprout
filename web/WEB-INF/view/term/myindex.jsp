                ...
                <c:when test="${terms != null}">
                    ...
                    <c:forEach items="${terms}" var="term">
                        ...
                        <li class="termItem round" viewCount="${term.viewCount}" korCount="${term.korTermCount}">
                            <div class="countBox">
                                <div class="korCount"><span class="countNum">${term.korTermCount}</span> <span class="countText">K</span></div>
                                <div class="viewCount"><span class="countNum">${term.viewCount}</span> <span class="countText">V</span></div>
                            </div>
                        ...
                        <c:if test="${term.korTermCount > 0}">
                            <c:forEach items="${term.korTerms}" var="korTerm" begin="0" end="2" varStatus="var">
                                <li class="korTermItem round row${var.count} action" termId="${term.id}" phrase="${korTerm.smallPhrase}">
                                    ...
                                    <span class="kor_phrase">${korTerm.smallPhrase}</span>
...
<script type="text/javascript">
    ...
    function initTermList(){
        ...
        loadTermList(getUrl('${term_url}'), '#termPage', param, '#termPage', function(){
            initFav();
        });
    }

    function goPaging(page){
        var url = getUrl('${term_url}');
        if($('#allParam').val()){
            url += '?' + $('#allParam').val();
        }
