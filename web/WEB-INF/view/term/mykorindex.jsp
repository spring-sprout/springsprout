...
<page:defaultpage banner_name="term-banner" selected_menu="Programming Terms" ajaxlogin_yn="N">
	<div id="content">
        ...
        <div id="termPage">
            <c:choose>
                <c:when test="${korTerms != null}">
                    <s2c:paging />
                    <ul id="termList">
                    <c:forEach items="${korTerms}" var="term">
                        <c:set var="termObject" value="${term}"/>
                        <li class="termItem round" viewCount="${term.devTerm.viewCount}" korCount="${term.devTerm.korTermCount}">
                            <div class="countBox">
                                <div class="korCount"><span class="countNum">${term.devTerm.korTermCount}</span> <span class="countText">K</span></div>
                                <div class="viewCount"><span class="countNum">${term.devTerm.viewCount}</span> <span class="countText">V</span></div>
                            </div>
                            <div class="termPhrase action" termId="${term.devTerm.id}" onclick="javascript:viewTerm(${term.devTerm.id});">${term.devTerm.phrase}</div>
                            <%--<div class="termFav action" termId="${term.id}" isFav="${member.favoriteTerms[termObject]}">--%>
                            <%--</div>--%>
                            <div class="termDetails round">${term.devTerm.smallDetails}</div>
                            <div class="termCreated">${term.devTerm.createdText}</div>
                            <div class="tagList">
                            <c:if test="${term.devTerm.tags != null}">
                                <ul>
                                    <c:forEach items="${term.devTerm.tags}" var="tag">
                        ...
                        <li class="korTermItem round row1 action" termId="${term.devTerm.id}" phrase="${term.smallPhrase}">
                            <span class="kor_vote_count">${term.vote}</span>
                            <span class="kor_vote_text">p</span>
                            <span class="kor_phrase">${term.smallPhrase}</span>
                            <img src="${term.member.avatar}&s=20"/>
                            <span class="kor_created">${term.createdText}</span>
                        </li>
                    </c:forEach>
                    </ul>
                </c:when>
...
<script type="text/javascript">
    ...
    function initTermList(){
        ...
        loadTermList(getUrl('mykor'), '#termPage', param, '#termPage', null);
    }
    
    function goPaging(page){
        var url = getUrl('mykor');
        if($('#allParam').val()){
            url += '?' + $('#allParam').val();
        }
